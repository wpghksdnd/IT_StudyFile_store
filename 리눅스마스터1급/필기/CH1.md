...
1. 운영체제
 1-1. 운영체제의 이해
	...
	# 운영체제 유형
	- 다중 교환(Muti-switching): 다수의 작어비 동시 실행, 포그라운드 프로그램만 동작
	...
	- 일괄 처리(Batch Processing): 여러 개의 작업을 묶어 한번에 처리
	...
	- 반환 시간(Turnaround Time): 작업이 제출되어 결과를 얻을 떄 까지의 총 소요시간
	- 신뢰도(Reliablity): 시스템이 얼마나 정확하게 작동되는지를 나타냄
	- 사용 가능도(Availability): 시스템에서 곧 사용할 수 있는 정도를 나타냄

	# 가상메모리
	- 하드디스크의 일부를 메모리(RAM)처럼 사용하는 것
	- 하드디스크에서 가상메모리로 쓰이는 영역을 "스왑 영역"이라고 한다.
    - 스와핑(Swaping): 메모리와 하드디스크 사이의 데이터 교환

    # 페이지 교체 알고리즘
        - FIFO(First-in Frist-Out): 먼저 적재된 페이지를 제거하는 알고리즘
        - LRU(Least Recently Used): 가장 오랫동안 참조되지 않은 페이지를 제거하는 알고리즘
        - LFU(Least Frequently Used): 최근에 가장 적게 사용된(참조된) 페이지를 제거하는 알고리즘
        - NUR(Not Used Recently): 참조비트와 변형비트를 사요하여 가장 오랫동안 참조되지 않은 페이지를 제거하는 알고리즘(LRU      알고리즘의 변형)
